...
- discount: 숫자만.
  ...
  (예: 10%, min_order 18,900원, 최대 10,000원
      → 18,900 × 0.1 = 1,890 → discount: 1890, special_condition: "10%할인 최대 10,000원")
...
== 땡겨요(TTANG) 추출 규칙 ==
- 고정 금액 쿠폰은 화면에 표시된 할인액을 그대로 discount에 기록.
  (5,000원 차감 계산 절대 금지 — 스크립트가 자동 처리함)
  % 할인 쿠폰은 위 공통 필드 규칙의 계산 방식(min_order × rate, cap at max) 적용.
